...
module.exports = function createHealthRouter({ pool }) {
  ...
  router.get('/health', async (_req, res) => {
    try {
      ...
      logger.error({ err: err?.message }, '[ia] Fallo al construir /api/health.');
  ...
  router.get('/health/ready', async (_req, res) => {
    try {
      ...
      logger.error({ err: err?.message }, '[ia] Fallo en /api/health/ready.');
  ...
  router.get('/health/breakers', (_req, res) => {
    try {
      ...
      logger.error({ err: err?.message }, '[ia] Fallo al leer breakers.');
